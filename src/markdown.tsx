...
import { ChannelMeta, GuildMeta, Message, Role, User, UserId } from './datatypes';
...
function renderTimestamp(timestamp: number): string {
...
const newRole = {
    ...
    match: function(source: any, state: State, lookbehind: any) {
        return /^<@&(\d{17,24})>/.exec(source);
    },
    parse: function(capture: any, recurseParse: any, state: State) {
    ...
    html: function(node: any, recurseOutput: any, state: State) {
...
const oldMention = {
    ...
    match: function(source: any, state: State, lookbehind: any) {
        ...
        let searchTerms = state.mentions.map(term => escapeRegExpForOrSearch(term.nickname));
...
const newMention = {
    ...
    parse: function(capture: any, recurseParse: any, state: State) {
        let mentionId = capture[1].toString()
        let mentionNickName = state.users[mentionId]?.nickname ?? null
        return {
            type: 'newMention',
            userId: capture[1],
...
const messageLink = {
  ...
  html: function(node: any, recurseOutput: any, state: State) {
      return `<a class="message-mention" href="?https://discord.com/channels/${node.guildId}/${node.channelId}/${node.messageId}">${CHANNEL_ICON} message link ${CHEVRON_ICON}${MESSAGEBUBBLE_ICON}</a>`;
  },
}
...
const aside = {
  ...
  html: function(node: any, recurseOutput: any, state: State) {
      return `<aside>${recurseOutput(node.content, state)}</aside>`;
  },
}
...
const oldEmoji = {
  ...
  match: function(source: any, state: State, lookbehind: any) {
      if (state.inlineEmojis.length === 0) {
          return null;
      }
      let searchTerms = state.inlineEmojis.map(term => escapeRegExpForOrSearch(term.name));
      return new RegExp(`^:(${searchTerms.join("|")}):`, 'i').exec(source);
  },
  parse: function(capture: any, recurseParse: any, state: State) {
    let url = null
    ...
    for (const emote of state.inlineEmojis) {
        if (emote.name == emojiName) {
            const newUrl = checkUrl(emote.imageUrl)
            if (newUrl !== "") {
                url = newUrl
            }
        }
...
//function openInGalleryTemplate(asset: Asset: any) {
//    if (asset == null) {
...
const newEmoji = {
  ...
  parse: function(capture: any, recurseParse: any, state: State) {
    ...
    for (const emote of state.inlineEmojis) {
        if (emote.id == paddedEmojiId) {
            const newUrl = checkUrl(emote.imageUrl)
            if (newUrl !== "") {
                url = newUrl
            }
            imageObj = emote.imageUrl
        }
    }
    return {
        type: 'newEmoji',
...
const textSpoiler = {
  ...
  html: function(node: any, recurseOutput: any, state: State) {
      return `<label class="spoiler"><input type="checkbox"><span>${node.content}</span></label>`;
  },
}
...
console.log(SimpleMarkdown.defaultRules.paragraph)
...
const parse = SimpleMarkdown.parserFor(rules as any);
var htmlOutput = SimpleMarkdown.outputFor(rules, 'html');




export function parseMarkdown(input: string, state: State) {
